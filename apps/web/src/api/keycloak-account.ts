...
function kcAccountBase(): string {
  const url = (import.meta.env.VITE_KEYCLOAK_URL as string | undefined) ?? 'http://localhost:8080'
  const realm = (import.meta.env.VITE_KEYCLOAK_REALM as string | undefined) ?? 'inktide'
