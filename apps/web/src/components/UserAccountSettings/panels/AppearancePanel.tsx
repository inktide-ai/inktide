...
function applyTheme(theme: AppearancePrefs['theme']) {
  ...
  root.setAttribute('data-theme', theme === 'system' ? prefer : theme)
}
...
export default function AppearancePanel({ onDirty, saveRef, onCancelRef }: Props) {
  ...
  // Apply theme immediately on change, save to localStorage
  function update<K extends keyof AppearancePrefs>(key: K, value: AppearancePrefs[K]) {
    setPrefs((p) => {
      const next = { ...p, [key]: value }
      savePrefs(next)
      if (key === 'theme') applyTheme(value as AppearancePrefs['theme'])
      return next
    })
  }

  // Appearance saves immediately via localStorage — no dirty/save flow needed
  ...
  useEffect(() => { applyTheme(prefs.theme) }, [])
