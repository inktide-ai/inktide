...
export function SidebarProjectItem({
  ...
  isSelected, isActive, onClick, className, layoutId,
}: SidebarProjectItemProps) {
  const safeAccent = HEX_COLOR_RE.test(accentColor) ? accentColor : '#6b7280'
  const usePill = isSelected && !!layoutId
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'relative flex items-center gap-2.5 px-2.5 py-[7px] rounded-lg text-left transition-colors w-full',
        isSelected
    ...
    >
      ...
      <div className="relative flex-1 min-w-0">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span
            className="flex-shrink-0 text-2xs px-1.5 py-px rounded"
            style={{ background: `${safeAccent}20`, color: safeAccent }}
          >
            bot
          </span>
        </div>
        <div className="text-body truncate opacity-60">/{slug}</div>
      </div>
    </button>
  )
}
