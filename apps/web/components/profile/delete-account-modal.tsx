  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="m-0 mb-4 text-[0.875rem] leading-relaxed text-(--text-muted)">
        ...
        <label className="block mb-1 text-[0.8125rem] font-medium text-(--text-secondary)">
          {userEmail
        ...
        <input
          ...
          className="mb-4 w-full rounded-[8px] border border-white/15 bg-white/[0.04] px-3 py-[9px] text-[0.875rem] text-(--text-primary) placeholder:text-(--text-muted) outline-none focus:border-[rgba(237,62,62,0.6)] transition-colors disabled:opacity-65"
        />

        {error && (
          <p className="m-0 mb-4 py-[0.625rem] px-3 rounded-[8px] text-[0.8125rem] leading-[1.45] text-[#ffc9c9] bg-[rgba(237,62,62,0.12)] border border-[rgba(237,62,62,0.35)]" role="alert">
            {error}
          </p>
        )}
        <div className="flex gap-3 justify-end flex-wrap">
          <button
            type="button"
            className="py-2 px-4 rounded-[10px] border border-white/15 bg-transparent text-(--text-primary) font-[var(--font-ui)] text-[0.875rem] font-medium cursor-pointer transition-[background] duration-150 ease hover:bg-white/[0.06] disabled:opacity-65 disabled:cursor-not-allowed"
            onClick={onClose}
            disabled={isDeleting}
          >
            Cancel
