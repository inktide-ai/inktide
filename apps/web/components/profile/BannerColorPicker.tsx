...
export function BannerColorPicker({ value, onChange }: BannerColorPickerProps) {
  ...
  return (
    <div className="relative inline-block" ref={rootRef}>
      <button
        type="button"
        className="group w-[72px] h-[72px] rounded-[12px] border border-white/10 cursor-pointer p-0 relative overflow-hidden transition-[border-color,box-shadow] duration-150 shadow-[0_4px_16px_rgba(0,0,0,0.4)] hover:border-white/25 hover:shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
      ...
      {open && (
        <div className="absolute top-[calc(100%+10px)] left-0 z-[200] w-60 bg-[#181C23] border border-[#272C33] rounded-[14px] p-4 shadow-[0_24px_64px_rgba(0,0,0,0.7)] flex flex-col gap-[0.875rem]">
          ...
          <div className="flex items-center gap-[0.375rem] bg-[#12161C] border border-[#2A2E32] rounded-[8px] px-3 py-[0.45rem] transition-[border-color,box-shadow] duration-150 focus-within:border-[rgba(237,62,62,0.5)] focus-within:shadow-[0_0_0_2px_rgba(237,62,62,0.08)]">
            <span className="font-[var(--font-mono,monospace)] text-[0.875rem] text-[var(--text-muted,rgba(255,255,255,0.45))] select-none">#</span>
            <input
              className="flex-1 bg-transparent border-none outline-none font-[var(--font-mono,monospace)] text-[0.875rem] text-[var(--text-primary,#fff)] tracking-[0.06em] uppercase"
              value={hexInput}
