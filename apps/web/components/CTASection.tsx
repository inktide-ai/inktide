  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          className="inline-flex items-center gap-[7px] px-[18px] py-[6px] mb-6 rounded-full text-sm font-semibold cursor-pointer bg-[#6C63FF15] [border:0.5px_solid_#6C63FF30] text-[#5248c8] dark:bg-[rgba(108,99,255,0.12)] dark:[border:0.5px_solid_rgba(108,99,255,0.28)] dark:text-[#b8b4ff]"
        >
          {t('cta.badge')}
          <span
            className="inline-block w-[22px] h-[22px] flex-shrink-0 bg-current"
            style={{ maskImage: 'url(/icons/badge-icon.svg)', maskSize: 'contain', maskRepeat: 'no-repeat', WebkitMaskImage: 'url(/icons/badge-icon.svg)', WebkitMaskSize: 'contain', WebkitMaskRepeat: 'no-repeat' }}
            aria-hidden
          />
        </button>

        <h2 className="[font-family:var(--font-heading)] text-[4rem] font-bold tracking-[-0.02em] leading-[1.1] text-[var(--text-primary)] mb-4 max-md:text-[2rem] max-[480px]:text-[1.75rem]">
          Boost d<span className="relative inline-block whitespace-nowrap">
            <img
              ...
              aria-hidden
            />
            <span className="relative z-[1] italic text-[#141413]">eveloper</span>
          </span>{' '}experience
        </h2>
