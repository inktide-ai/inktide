...
export default function Navigation() {
  ...
  const avatarInitial = (user?.nickname?.trim() || user?.userName || userEmail || '?')
  ...
  return (
    <>
      ...
      <nav className={cn(
        ...
        <div className="max-w-[1400px] h-full mx-auto flex items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <img src="/logo/icon_main_white.svg" className="h-7 w-auto object-contain -mt-1 transition-[filter] duration-[250ms]" alt="" aria-hidden />
            </button>
          ...
          <div className="hidden lg:flex items-center gap-[0.625rem] flex-shrink-0">
            ...
            {isLoggedIn ? (
              <>
                ...
                >
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--accent-red)] text-xs font-bold text-white overflow-hidden flex-shrink-0" aria-hidden>
                    {user?.pictureUrl
      ...
      <div className={cn(
        ...
        'bg-[#111110] border-l border-white/[0.07]',
        ...
        <div className="flex flex-col gap-[0.625rem] p-5 border-t border-white/[0.06] flex-shrink-0">
          {isLoggedIn ? (
            <>
              ...
              <button
                type="button"
                className="w-full py-[0.8rem] px-5 bg-[#f5f4f0] border-none rounded-[10px] text-[#141413] font-[family-name:var(--font-ui)] text-[0.925rem] font-semibold cursor-pointer transition-[background] duration-[180ms] hover:bg-white"
                onClick={() => { closeMobile(); router.push('/home') }}
              >
                {t('nav.goToProfile')}
              </button>
              <button
