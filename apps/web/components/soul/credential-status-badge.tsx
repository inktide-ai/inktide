...
export function CredentialStatusBadge({ status, error, className }: CredentialStatusBadgeProps) {
  if (status === 'untested') {
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[0.75rem] font-medium
  ...
  if (status === 'verified') {
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[0.75rem] font-medium
          bg-[rgba(34,197,94,0.1)] text-[#22c55e] ${className ?? ''}`}
      >
  ...
  return (
    <span
      className={`inline-flex flex-col gap-0.5 rounded-md px-2 py-1 text-[0.75rem] font-medium
        bg-[rgba(239,68,68,0.1)] border border-[rgba(239,68,68,0.2)] text-[#f87171] ${className ?? ''}`}
    >
...
/** Derives CredentialStatus from stored credential fields */
