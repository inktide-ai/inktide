...
import { NODE_DEFINITIONS, type PipelineNodeType } from './node-definitions'
...
function portHandleStyle(dataType: string): React.CSSProperties {
  const color = TYPE_COLOR[dataType] ?? 'rgba(255,255,255,0.55)'
  return {
    width: 10, height: 10, borderRadius: '50%',
    background: `radial-gradient(circle at center, #fff 38%, ${color} 38%)`,
...
export function PipelineNode({ data, selected }: NodeProps) {
  ...
  const border = selected ? 'var(--border-strong)' : 'var(--border-subtle)'

  return (
    <div style={{
      minWidth: 148,
      background: 'hsla(var(--bg-2), 1)',
      ...
      boxShadow: selected
        ? `0 0 0 1px var(--border-default), 0 4px 20px rgba(0,0,0,0.6)`
        : '0 2px 12px rgba(0,0,0,0.4)',
      transition: 'border-color 0.15s, box-shadow 0.15s',
    }}>
      {/* Single row: icon + label + type badge */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 10px 9px 11px' }}>
        <span style={{
      ...
      {def.inputs.map((p, i) => (
        <Handle
          ...
          id={p.name}
          style={{ ...portHandleStyle(p.dataType), top: `calc(50% + ${(i - (def.inputs.length - 1) / 2) * 14}px)`, left: -5 }}
        />
      ))}
      ...
      {def.outputs.map((p, i) => (
        <Handle
          ...
          id={p.name}
          style={{ ...portHandleStyle(p.dataType), top: `calc(50% + ${(i - (def.outputs.length - 1) / 2) * 14}px)`, right: -5 }}
        />
      ))}
    </div>
  )
