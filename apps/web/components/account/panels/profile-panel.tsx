...
import { getKcProfile, updateKcProfile, getKcCredentials, type KcAccountProfile, type KcCredential } from '@/api/keycloak-account'
  ...
  return (
    <>
      ...
      <div className="flex flex-col">
        <div className="flex items-center">
          <button
            ...
            title={storageEnabled === false ? 'Storage not configured' : 'Change avatar'}
          >
            {user?.pictureUrl
            ...
            {avatarBusy && (
              <span className="absolute inset-0 flex items-center justify-center rounded-full bg-black/45 text-[14px] text-white">…</span>
            )}
          </button>
          <div className="ml-[20px] w-[250px]">
            <label className="mb-[4px] block text-[14px] leading-[16px] text-[var(--text-secondary)]">Preferred name</label>
            <input
              value={nameDraft}
              ...
              className={[
                'w-full rounded-[8px] border px-[10px] py-[7px] text-[14px] text-[var(--text-heading)] outline-none transition-colors',
        ...
        {storageEnabled !== false && (
          <div className="mt-[8px] text-[14px]">
            ...
            <span className="text-[var(--text-secondary)]">your avatar</span>
          </div>
        )}
        <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={onAvatarPick} />
        {avatarError && (
          <p className="mt-2 text-[10px] text-[var(--danger-text)]">{avatarError}</p>
        )}
      </div>
      ...
      <div>
        ...
        <SecurityRow
          label="Password"
          value={hasPassword ? 'Update your account password' : 'Set a password for your account'}
          action={
            <button
              type="button"
        ...
        <div className="h-[24px]" />
        {/* Member since */}
        <SecurityRow label="Member since" value="May 18, 2026" />
      </div>
      ...
      <div className="divide-y divide-[var(--settings-card-border)]">
        <SecurityRow
          label="User ID"
          value={
            <span className="font-mono text-[14px] font-normal text-pretty text-[var(--text-secondary)]">
              {user?.userId ?? '—'}
            </span>
          }
          action={
