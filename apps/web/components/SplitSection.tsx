...
export default function SplitSection({ heading, description, reverse = false, imageDark, imageAlt = '' }: SplitSectionProps) {
  ...
  const visual = (
    <div
      ...
      className={cn(
        ...
        (reverse ? rightVisible : leftVisible)
          ? 'opacity-100 translate-x-0'
          : reverse ? 'opacity-0 translate-x-8' : 'opacity-0 -translate-x-8',
      )}
    >
      {imageDark ? (
  ...
  const content = (
    <div
      ...
      className={cn(
        ...
        (reverse ? leftVisible : rightVisible)
          ? 'opacity-100 translate-x-0'
          : reverse ? 'opacity-0 -translate-x-8' : 'opacity-0 translate-x-8',
      )}
    >
      <h2 className="text-3xl lg:text-4xl font-bold text-[var(--text-primary)] leading-tight">
        {heading}
      </h2>
  ...
  return (
    <section className="py-20 px-6">
      <div className={cn(
        'max-w-[1200px] mx-auto flex flex-col lg:flex-row items-center gap-12',
