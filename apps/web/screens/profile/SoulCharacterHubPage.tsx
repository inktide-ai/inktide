...
import { type HubTabId, type CardLayout, useSoulHubLayout } from '../../hooks/useHubLayout'
...
function tagStyle(cat: TagCategory): React.CSSProperties {
  const [rgb, text] = TAG_PALETTE[cat]
  return { background: `rgba(${rgb}, 0.1)`, borderColor: `rgba(${rgb}, 0.2)`, color: text }
}

interface TagItem { label: string; style: React.CSSProperties }
...
export default function SoulCharacterHubPage({ soulId }: { soulId: string }) {
  ...
  }, [soulId, selected, selectedId, selectCard])
  ...
  const soulLinks = buildSoulLinks(soulId)
  const LINK_MAP = Object.fromEntries(soulLinks.map(l => [l.tabId, l])) as Record<SoulTabId, (typeof soulLinks)[0]>
    ...
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        >
          ...
          >
            {layout.map(item => {
              const link = LINK_MAP[item.i as SoulTabId]
              const meta = SOUL_CARD_META[item.i as SoulTabId]
              ...
              return (
                <div key={item.i} style={{ position: 'relative', width: '100%', height: '100%' }}>
                  {isEditMode ? (
                    <div
                      data-tabid={item.i}
                      className="tab-card tab-card-editing"
                    >
