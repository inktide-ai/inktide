...
import { PRESET_SCENES, type PresetScene } from '@/components/profile/tabs/preset-scene-card'
import { NewSceneModal } from '@/components/profile/tabs/new-scene-modal'
import { getEffectiveTagLabel, getSceneDisplayTitle, BUILTIN_SCENE_TAGS } from '@/components/profile/tabs/scene-tag-utils'
import { reorderScene, type AiCardSceneResponse } from '@/api/soul'
...
function Slider({ label, value, onChange, unit = '%' }: { label: string; value: number; onChange: (v: number) => void; unit?: string }) {
  return (
    <div className="flex items-center gap-3">
      <span className="w-20 shrink-0 text-[14px] text-[var(--text-tertiary)]">{label}</span>
      <input
        type="range" min={0} max={100} value={value}
        onChange={e => onChange(Number(e.target.value))}
        className="flex-1 h-1 accent-[var(--accent-primary)] cursor-pointer"
      />
      <span className="w-10 text-right text-[14px] text-[var(--text-secondary)]">{value}{unit}</span>
    </div>
  )
}
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-auto p-5 min-w-0">
        ...
        <div className="relative min-h-[260px] flex-1 overflow-hidden rounded-2xl border border-[var(--border-subtle)]" style={previewBg}>
          ...
          <div className="absolute left-3 top-3 flex items-center gap-1.5 rounded-full border border-white/10 bg-black/50 px-3 py-1 backdrop-blur-sm">
            ...
            <span className="text-[12px] font-medium text-white">Active Scene</span>
            <span className="text-[12px] text-white/60">{activeLabel}</span>
          </div>
          {/* Live Preview badge */}
          <div className="absolute right-3 top-3 rounded-full border border-white/10 bg-black/50 px-3 py-1 backdrop-blur-sm">
            <span className="text-[12px] font-medium text-white/80">Live Preview</span>
          </div>
          {/* Placeholder when no scene */}
          {!activeItem && (
            <div className="absolute inset-0 flex items-center justify-center">
              <p className="text-[14px] text-[var(--text-tertiary)]">No scene selected</p>
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)]/40">
          <div className="border-b border-[var(--border-subtle)] px-4 py-3">
            <p className="text-[14px] font-semibold text-[var(--text-primary)]">Scene Controls</p>
            <p className="text-[14px] text-[var(--text-tertiary)]">Adjust and customize your current scene.</p>
          </div>
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 px-4 py-4">
            ...
            <div className="flex items-center gap-3">
              <span className="w-20 shrink-0 text-[14px] text-[var(--text-tertiary)]">Color Filter</span>
              <select
                value={colorFilter}
                onChange={e => setColorFilter(e.target.value)}
                className="flex-1 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-1.5 text-[14px] text-[var(--text-primary)] outline-none focus:border-[var(--accent-primary)]"
              >
                {COLOR_FILTERS.map(f => <option key={f.label}>{f.label}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-3">
              <span className="w-20 shrink-0 text-[14px] text-[var(--text-tertiary)]">Bg Animation</span>
              <button
                type="button"
      ...
      <div className="flex w-[280px] shrink-0 flex-col overflow-hidden border-l border-[var(--border-subtle)]">
        ...
        <div className="shrink-0 px-4 py-4">
          <p className="text-[14px] font-semibold text-[var(--text-primary)]">All Scenes</p>
          <p className="mt-0.5 text-[14px] text-[var(--text-tertiary)]">Choose or create the perfect background.</p>
        </div>
        ...
        <div className="shrink-0 flex gap-0.5 border-b border-[var(--border-subtle)] px-3 pb-2">
          {(['All', 'Default', 'Custom', 'Animated'] as TabFilter[]).map(t => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                'h-7 rounded-md px-2.5 text-[14px] font-medium transition-colors',
                tab === t
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {loading ? (
            <p className="py-6 text-center text-[14px] text-[var(--text-tertiary)]">Loading…</p>
          ) : (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleSceneDragEnd}>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <p className="text-[12px] font-medium text-[var(--text-tertiary)] group-hover:text-[var(--text-secondary)]">Upload Scene</p>
                  <p className="text-[10px] text-[var(--text-tertiary)]/60">Image, Video, or GIF</p>
                </button>
              </div>
            </DndContext>
          )}
