    ...
    <div className="px-5 py-4 space-y-3">
      <div className="overflow-hidden rounded-lg border border-[var(--border-card)]">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-divider)]">
          <div>
            <p className="text-body font-semibold text-[var(--text-primary)]">Connected channels</p>
            <p className="text-body text-[var(--text-tertiary)] mt-0.5">Connect your streaming and social platforms to your soul.</p>
          </div>
          <div role="tablist" className="flex items-center gap-1">
            {FILTERS.map((f) => (
              <button
                ...
                onClick={() => setFilter(f)}
                className={cn(
                  'h-7 whitespace-nowrap rounded-full px-3 text-sm font-medium transition-all duration-150 outline-none',
                  filter === f
