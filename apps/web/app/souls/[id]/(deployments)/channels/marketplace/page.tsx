        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="flex flex-col items-stretch justify-between gap-4 md:flex-row md:items-center md:gap-2">
            <div className="flex flex-col gap-2">
              ...
              <span className="text-[0.875rem] text-[var(--text-secondary)] pl-9">
            ...
            <div className="flex gap-2 flex-wrap">
              {categories.map(cat => (
                <button
                  ...
                  className={`h-9 px-3 text-[0.8125rem] rounded-lg border transition-colors ${
                    categoryFilter === cat
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {/* badge */}
                    <span className="mt-0.5 inline-flex items-center gap-1 rounded-full border border-[var(--border-subtle)] px-2 py-0.5 text-[0.6875rem] font-medium text-[var(--text-tertiary)] shrink-0">
                      {connector.category}
                    </span>
                  </div>

                  <div className="flex flex-col gap-1 flex-1">
                    <p className="text-[0.9375rem] font-semibold text-[var(--text-heading)]">{connector.name}</p>
                    <p className="text-[0.8125rem] text-[var(--text-secondary)] leading-snug">{connector.description}</p>
                  </div>

                  {connector.is_available ? (
                    isInstalled ? (
                      <span className="inline-flex items-center gap-1.5 text-[0.8125rem] font-medium text-green-500">
                        <Check size={14} />
                        Installed
                      </span>
                    ) : (
                      <button
                        ...
                        disabled={!!installing}
                        className="inline-flex h-8 items-center justify-center rounded-md bg-[var(--text-primary)] px-4 text-[0.8125rem] font-medium text-[var(--bg-0)] transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isBusy ? 'Installing…' : 'Install'}
                      </button>
                    )
                  ) : (
                    <span className="inline-flex items-center gap-1.5 text-[0.8125rem] font-medium text-[var(--text-tertiary)]">
                      <Clock size={14} />
                      Coming soon
                    </span>
                  )}
