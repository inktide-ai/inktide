...
const PLATFORM_COLORS: Record<string, { bg: string; fg: string; label: string }> = {
  ...
  twitch:   { bg: 'rgba(145,70,255,0.15)',  fg: '#9146ff', label: 'Twitch'   },
...
export default function PublicSoulPage() {
  const params = useParams()
  const slug = params.id as string

  const [soul, setSoul] = useState<PublicAiCardResponse | null>(null)
  ...
  if (error || !soul) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center bg-[var(--bg-0)]">
        ...
        <Link href="/" className="text-[0.875rem] text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
          ← Back to Inktide
        </Link>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-0)]">
      ...
      <div className="mx-auto flex max-w-lg flex-col items-center px-6 pb-16 text-center">
        ...
        <p className="mt-0.5 text-[14px] text-[var(--text-tertiary)]">@{soul.slug}</p>

        {soul.description && (
          <p className="mt-2 text-[0.9375rem] text-[var(--text-secondary)]">{soul.description}</p>
        )}
        ...
        {soul.platforms.length > 0 && (
          <div className="mt-5 w-full">
            <p className="mb-2 text-[12px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">Connected to</p>
        ...
        {soul.personality && (
          <div className="mt-5 w-full rounded-2xl border border-[var(--border-card)] bg-[var(--surface-card)] p-4 text-left">
            <p className="mb-1.5 text-[12px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">Personality</p>
            <p className="text-[0.875rem] leading-relaxed text-[var(--text-secondary)]">{personalityExcerpt}</p>
          </div>
        )}
        ...
        <div className="mt-8 flex items-center justify-center gap-3 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] px-6 py-4">
          <span className="text-[0.8125rem] text-[var(--text-tertiary)]">Powered by</span>
          <span className="text-[0.9375rem] font-semibold text-[var(--text-primary)]">Inktide</span>
          <span className="text-[var(--text-tertiary)]">·</span>
          <Link
            href="/pricing"
            className="text-[0.875rem] font-semibold text-[var(--accent-primary)] hover:opacity-80"
          >
            Create your own →
          </Link>
        </div>
