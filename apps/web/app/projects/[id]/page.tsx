  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--border-card)]">
      <div className="flex items-center justify-between border-b border-[var(--border-subtle)] px-5 py-3.5">
        ...
        <Link
          href={href}
          className="flex items-center gap-0.5 text-[14px] text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)]"
        >
...
function MetaRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-[var(--border-subtle)] py-2.5 last:border-0">
      <span className="shrink-0 text-[14px] text-[var(--text-tertiary)]">{label}</span>
      <span className="text-right text-[14px] font-medium text-[var(--text-primary)]">{value}</span>
    </div>
  )
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex h-5 items-center rounded-full border px-2 text-[12px] font-medium ${cls}`}>
      {label}
    </span>
  )
}
    ...
    <div className="mx-auto max-w-[1000px] px-6 py-8">
          ...
          {editing ? (
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={2}
                className="w-full resize-none rounded-xl border border-[var(--border-subtle)] bg-[hsla(var(--bg-1),_1)] px-3 py-2 text-[14px] text-[var(--text-primary)] outline-none focus:border-[var(--accent-primary)] placeholder:text-[var(--text-tertiary)]"
              />
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={handleSave}
                  className="h-8 rounded-lg bg-[var(--accent-primary)] px-4 text-[14px] font-medium text-white hover:bg-[var(--accent-hover)] disabled:opacity-50"
                >
                  {saving ? 'Saving…' : 'Save'}
                </button>
                <button
                  ...
                  className="h-8 rounded-lg border border-[var(--border-subtle)] bg-[hsla(var(--bg-1),_1)] px-4 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-2)]"
                >
                  Cancel
                </button>
              </div>
            ...
            <>
              ...
              {project.description && (
                <p className="mt-1 text-[14px] text-[var(--text-secondary)]">{project.description}</p>
              )}
            </>
          )}
        </div>
        ...
        {!editing && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-9 items-center gap-1.5 rounded-xl border border-[var(--border-subtle)] bg-[hsla(var(--bg-1),_1)] px-3 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-2)]"
            >
              <Pencil size={13} /> Edit
            </button>
            <button
              ...
              className="flex h-9 items-center gap-1.5 rounded-xl border border-[var(--border-subtle)] bg-[hsla(var(--bg-1),_1)] px-3 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-2)] disabled:opacity-40"
            >
            ...
            <button
              ...
              className="flex h-9 items-center gap-1.5 rounded-xl bg-[var(--accent-primary)] px-4 text-[14px] font-medium text-white hover:bg-[var(--accent-hover)]"
            >
          ...
          {previewUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-b from-[var(--surface-2)] to-[var(--bg-deeper,var(--bg-0))]">
              <div className="flex flex-col items-center gap-2 text-[var(--text-tertiary)]">
                ...
                <span className="text-[14px]">No scene configured</span>
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between gap-3 px-5 py-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[14px] font-semibold text-white">Sandbox</span>
              </div>
            ...
            <button
              ...
              className="flex h-9 shrink-0 items-center gap-1.5 rounded-xl bg-white/10 px-4 text-[14px] font-medium text-white backdrop-blur-sm hover:bg-white/20"
            >
        ...
        <SectionCard icon={Cpu} title="Character" href={`${base}/character`}>
          {!soul ? <NoSoul /> : (
            <div className="-my-2.5">
              <MetaRow label="Soul" value={soul.name} />
              {soul.llm_model && (
                <MetaRow
                  label="Model"
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 rounded-lg border border-[var(--border-subtle)] px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[12px] text-[var(--text-tertiary)]">3D Model</p>
                </div>
                {activeModel && (
              ...
              <div className="flex items-center gap-3 rounded-lg border border-[var(--border-subtle)] px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[12px] text-[var(--text-tertiary)]">Background</p>
                </div>
                {activeScene && (
                  <span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] text-emerald-400">
                    Active
      ...
      <div className="mb-6 grid gap-6 md:grid-cols-2">
        ...
        <SectionCard icon={Radio} title="Channels" href={`${base}/channels`}>
          {!soul ? <NoSoul /> : activeChannels.length === 0 ? (
            <p className="text-[14px] text-[var(--text-tertiary)]">No active channels yet.</p>
          ) : (
            <div className="space-y-2">
              {activeChannels.slice(0, 4).map(ch => (
                <div key={ch.id} className="flex items-center gap-2.5 rounded-lg border border-[var(--border-subtle)] px-3 py-2">
                  ...
                  <span className="text-[14px] font-medium capitalize text-[var(--text-primary)]">{ch.platform}</span>
                  <span className="truncate text-[14px] text-[var(--text-secondary)]">· {ch.channel_name}</span>
                </div>
              ))}
              {activeChannels.length > 4 && (
                <p className="text-[12px] text-[var(--text-tertiary)]">+{activeChannels.length - 4} more</p>
              )}
            </div>
          )}
        </SectionCard>

        <SectionCard icon={Cpu} title="Soul" href="/souls">
          {!project.active_soul ? (
            <div className="space-y-4">
              <div>
                <p className="text-[14px] font-medium text-[var(--text-primary)]">Select Soul</p>
                <p className="mt-1 text-[14px] text-[var(--text-secondary)]">
                  Connect a personality, memory and behavior layer to this project.
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="h-8 rounded-lg border border-[var(--border-subtle)] bg-transparent px-3 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-1)]"
                >
                  Select
                </button>
                <Link
                  href="/souls"
                  className="flex h-8 items-center rounded-lg bg-white px-3 text-[14px] font-medium text-black hover:bg-[#f2f2f2]"
                >
                  Browse Souls
                </Link>
              </div>
            ...
            <div className="flex items-center gap-3">
              {project.active_soul.avatar_url ? (
                ...
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--surface-2)] text-[14px] font-medium text-[var(--text-secondary)]">
                  {project.active_soul.name[0].toUpperCase()}
                </div>
              )}
              <span className="text-[14px] font-medium text-[var(--text-primary)]">{project.active_soul.name}</span>
              <button
                type="button"
                onClick={() => setSoulPickerOpen(true)}
                className="ml-auto h-8 rounded-lg border border-[var(--border-subtle)] bg-transparent px-3 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-1)]"
              >
                Change
              </button>
            </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              {obsUrl ? (
                <div className="flex shrink-0 items-center gap-2">
                  <code className="max-w-[300px] truncate rounded-lg border border-[var(--border-subtle)] bg-black/30 px-3 py-1.5 font-mono text-[12px] text-cyan-300/90">
                    {obsUrl}
                  </code>
                  <button
                    type="button"
                    onClick={copyObsUrl}
                    className="flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 text-[14px] font-medium text-[var(--text-primary)] hover:bg-[var(--surface-2)]"
                  >
                ...
                <Link
                  href={`${base}/obs`}
                  className="flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 text-[14px] text-[var(--text-secondary)] hover:bg-[var(--surface-2)]"
                >
