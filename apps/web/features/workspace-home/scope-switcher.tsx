...
const panelClass = cn(
  ...
  'bg-[var(--menu-panel-bg)] text-[14px] text-[var(--text-primary)] shadow-md',
)
...
function ProjectInitial({ project }: { project: ProjectListItem }) {
  ...
  return (
    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-[4px] bg-[var(--surface-2)] text-[10px] font-bold uppercase text-[var(--text-secondary)]">
      {project.name[0]}
    </span>
  )
}
  ...
  return (
    <DropdownMenu.Root open={open} onOpenChange={handleOpenChange}>
      <DropdownMenu.Trigger asChild>
        <button
          type="button"
          className="flex items-center gap-1 rounded-lg bg-[var(--bg-0)] px-2.5 py-1 text-[14px] font-medium text-[var(--text-primary)] outline-none transition-colors hover:bg-[var(--surface-1)] focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)]"
        >
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={panelClass} align="start" sideOffset={6}>
          {/* Search */}
          ...
          <div className="max-h-[260px] overflow-y-auto p-1">
            {loading ? (
              ...
              <p className="px-2 py-3 text-center text-[12px] text-[var(--text-tertiary)]">
          ...
          <div className="border-t border-[var(--border-default)] p-1">
            ...
            >
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-[4px] border border-dashed border-[var(--border-default)] text-[14px] leading-none">
                +
              </span>
