...
import { getCard, type ChannelResponse } from '@/features/soul'
...
function AccordionSection({
  ...
  title: string; badge?: string; badgeColor?: 'default' | 'green' | 'yellow'
  statusIcon?: React.ReactNode; children: React.ReactNode; defaultOpen?: boolean
}) {
  ...
  return (
    <div className="border-t border-[var(--border-subtle)]">
      ...
      >
        <ChevronRight size={14} className={cn('shrink-0 text-[var(--text-tertiary)] transition-transform duration-150', open && 'rotate-90')} />
        <span className="flex-1 text-body font-medium text-[var(--text-primary)]">{title}</span>
        {badge && (
          <span className={cn(
...
function CheckItem({ ok, label, sub }: { ok: boolean; label: string; sub: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)]/40 px-4 py-3">
      <span className={cn('mt-[3px] h-2 w-2 shrink-0 rounded-full', ok ? 'bg-green-400 shadow-[0_0_6px_rgba(74,222,128,0.5)]' : 'bg-[var(--text-tertiary)]/30')} />
      <div>
        <p className="text-body font-medium text-[var(--text-primary)]">{label}</p>
        <p className={cn('mt-0.5 text-body', ok ? 'text-[var(--text-tertiary)]' : 'text-yellow-400/80')}>{sub}</p>
      </div>
    </div>
  )
}
...
function Meta({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mb-0.5 text-body text-[var(--text-tertiary)]">{label}</p>
      <div className="text-body font-medium text-[var(--text-primary)]">{children}</div>
    </div>
  )
}
            ...
            <div>
              ...
              {channels === null ? (
                ...
                <select value={selectedChannelId ?? ''} onChange={e => setSelectedChannelId(e.target.value || null)}
                  className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-1.5 text-body text-[var(--text-primary)] outline-none focus:border-[var(--accent-primary)]">
                  {channels.map(ch => (
                    <option key={ch.id} value={ch.channel_id ?? ''}>{ch.platform} · {ch.channel_name}</option>
                  ))}
                </select>
              )}
            </div>
            ...
            {[
              ...
              <div key={n} className="flex items-start gap-3 rounded-xl px-3 py-3 transition-colors hover:bg-[var(--surface-1)]/50">
                ...
                <div className="pt-0.5">
                  ...
                  <p className="mt-0.5 text-body leading-relaxed text-[var(--text-secondary)]">{desc}</p>
                  {code && <code className="mt-2 inline-block select-all rounded-lg border border-[var(--border-subtle)] bg-black/40 px-3 py-1.5 font-mono text-xs text-cyan-300/90">{code}</code>}
                </div>
              </div>
            ))}
          </div>
...
export default function SoulProjectObsPage() {
  ...
  return <ObsContent character={character} cardId={cardId} />
}
