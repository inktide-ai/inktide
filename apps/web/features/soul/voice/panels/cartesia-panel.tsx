...
import { CARTESIA_MODELS, stripModelPrefix } from '@/shared/data/voice-models'
  ...
  return (
    <>
      ...
      <section className="mb-6">
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] divide-y divide-[var(--border-subtle)]">
          <SelectField
            ...
            hint={!apiKey.trim() ? t('panels.hintEnterKey') : voicesLoading ? t('panels.loadingVoices') : t('panels.hintCartesiaVoices')}
            value={tts.voiceId ?? ''}
          ...
          <SelectField label={t('panels.model')} value={stripModelPrefix(tts.modelId, 'cartesia') ?? 'sonic-2'} onChange={(v) => patch({ modelId: v ? `cartesia/${v}` : null })}>
      ...
      <section>
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-1)] divide-y divide-[var(--border-subtle)]">
          <ParamRow icon={<SpeedIcon />} name={t('panels.speed')} desc={t('panels.speedPlayback')} min={0.0} max={2.0} step={0.05} decimals={2} value={Math.min(2.0, Math.max(0.0, tts.speed))} onChange={(v) => patch({ speed: v })} format={(v) => `${v.toFixed(2)}×`} />
        </div>
      </section>
    </>
  )
