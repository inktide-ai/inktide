...
import { BUILTIN_SCENE_TAGS, getSceneDisplayTitle, mergeSceneTagPickOptions } from './scene-tag-utils'
...
export function SceneSettings({ character, scene, cardId, onBack, onScenesChanged, onSceneReplaced, onSceneDeleted }: SceneSettingsProps) {
  ...
  useEffect(() => {
    ...
    listCustomSceneTags(cardId).then(c => setTagPickOptions(mergeSceneTagPickOptions(c))).catch(() => setTagPickOptions([...BUILTIN_SCENE_TAGS]))
  }, [cardId])
  ...
  async function handleReplace(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      try { await deleteCardScene(cardId, previousId) } catch (delErr) { setError(delErr instanceof Error ? delErr.message : t('settings.errorDelete')); onScenesChanged(); return }
  ...
  async function handleUploadNew(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true)
    setError(null)
    try { await executePresignedUpload(new CardSceneUploader(cardId), file); onScenesChanged() }
    catch (err) { setError(err instanceof Error ? err.message : t('settings.errorUpload')) }
  ...
  async function handleDelete() {
    ...
    setRemoving(true)
    setError(null)
    try { await deleteCardScene(cardId, scene.id); onScenesChanged(); onSceneDeleted() }
    catch (err) { setError(err instanceof Error ? err.message : t('settings.errorDelete')) }
