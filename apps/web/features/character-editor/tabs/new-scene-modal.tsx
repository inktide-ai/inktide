      ...
      <div className="w-full max-w-[480px] overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-0)] shadow-2xl animate-[pageIn_0.18s_cubic-bezier(0.2,0.9,0.2,1)]">
        ...
        <div className="flex flex-col gap-5 px-5 py-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-body font-medium text-[var(--text-secondary)]">{t('modal.nameLabel')}</label>
            <input
              autoFocus
              maxLength={40}
              value={name}
              ...
              className="w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-body text-[var(--text-primary)] outline-none transition-colors placeholder:text-[var(--text-tertiary)] focus:border-[var(--accent-primary)]"
            />
            <span className="text-right text-xs text-[var(--text-tertiary)]">{name.length}/40</span>
          </div>

          {/* Upload */}
          <div className="flex flex-col gap-1.5">
            <label className="text-body font-medium text-[var(--text-secondary)]">{t('modal.imageLabel')}</label>
            {preview ? (
              <div className="relative h-[130px] w-full overflow-hidden rounded-lg border border-[var(--accent-primary)]/50">
                ...
                <button
                  ...
                  className="absolute bottom-2 right-2 rounded-md border border-[var(--border-subtle)] bg-black/60 px-2.5 py-1 text-xs text-[var(--text-secondary)] backdrop-blur-sm transition-colors hover:text-[var(--text-primary)]"
                >
                  {t('modal.change')}
                </button>
              </div>
              ...
              >
                ...
                <p className="text-body font-medium text-[var(--text-secondary)]">{t('modal.uploadTitle')}</p>
                <p className="text-body text-[var(--text-tertiary)]">{t('modal.uploadSub')}</p>
              </div>
            )}
          ...
          {/* Tag */}
          <div className="flex flex-col gap-1.5">
            <label className="text-body font-medium text-[var(--text-secondary)]">{t('modal.tagLabel')}</label>
          ...
          {/* Description */}
          <div className="flex flex-col gap-1.5">
            <label className="flex items-center gap-1.5 text-body font-medium text-[var(--text-secondary)]">
              {t('modal.descLabel')}
              <span className="text-xs font-normal text-[var(--text-tertiary)]">{t('modal.optional')}</span>
            </label>
            <textarea
              maxLength={120}
              value={description}
              ...
              rows={3}
              className="w-full resize-none rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] px-3 py-2 text-body text-[var(--text-primary)] outline-none transition-colors placeholder:text-[var(--text-tertiary)] focus:border-[var(--accent-primary)]"
            />
            <span className="text-right text-xs text-[var(--text-tertiary)]">{description.length}/120</span>
          </div>
        ...
        <div className="flex gap-2 border-t border-[var(--border-subtle)] px-5 py-4">
          <button
            ...
            disabled={uploading}
            onClick={onClose}
            className="flex-1 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-2)] py-2 text-body font-medium text-[var(--text-secondary)] transition-colors hover:enabled:bg-[var(--surface-2)]/80 disabled:opacity-40"
          >
            {t('modal.cancel')}
          </button>
          <button
            ...
            className="flex-[2] rounded-lg bg-[var(--accent-primary)] py-2 text-body font-medium text-white transition-colors hover:enabled:bg-[var(--accent-hover)] disabled:opacity-40"
          >
