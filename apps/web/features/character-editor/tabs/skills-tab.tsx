...
// ── Text input cell (for name/slug — like InputCell but type=text) ───────────
function TextCell({ label, value, onChange, readOnly, accent, dark }: {
...
// Like the Figma Appearance grid: dark box, icon/label on left, value on right
function InputCell({ label, value, onChange, min, max, step = 1, unit, disabled }: {
...
// ── Select cell (same dark-box style) ────────────────────────────────────────
...
// ── Checkbox row — Figma "Clip content" style: [✓] Label ─────────────────────
...
// ── Grid wrapper with optional fieldset semantics ─────────────────────────────
const SR_ONLY: React.CSSProperties = {
  position: 'absolute', width: 1, height: 1, overflow: 'hidden',
  clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap',
}

function InputGrid({ legend, children }: {
