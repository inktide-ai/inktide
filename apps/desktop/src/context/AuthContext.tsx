...
export function AuthProvider({ keycloak, children }: { keycloak: Keycloak; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Avatar fetch should run when login state or user identity changes — not on every `user` field update.
...
/** Hook is intentionally exported next to the provider (common React pattern). */
// eslint-disable-next-line react-refresh/only-export-components -- see JSDoc above
