...
namespace Chimera.API.Synapse.Infrastructure.Session;

/// <summary>
/// Redis-backed conversation history.
/// Each channel keeps a capped list of JSON-serialised <see cref="ConversationTurn"/> entries.
/// Key format: <c>chimera:session:{channelId}</c>, TTL: 24 h.
/// </summary>
public sealed class RedisConversationHistoryRepository : IConversationHistoryRepository
{
    ...
    private static string BuildKey(string channelId) => $"chimera:session:{channelId}";
}
