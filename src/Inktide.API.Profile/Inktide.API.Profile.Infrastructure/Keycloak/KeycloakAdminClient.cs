...
public sealed class KeycloakAdminClient : IKeycloakAdminClient
{
    ...
    public KeycloakAdminClient(IHttpClientFactory httpFactory, KeycloakAdminSettings settings, ILogger<KeycloakAdminClient> logger)
    {
    ...
    // Creates a fresh client per operation so the handler pool can rotate DNS entries.
    private HttpClient CreateHttp() => _httpFactory.CreateClient(nameof(KeycloakAdminClient));
    ...
    public async Task<(bool Success, string? ErrorMessage)> TryDeleteUserAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        var token = await RequestTokenAsync(http, baseUrl, ct).ConfigureAwait(false);
        if (token is null)
        ...
        catch (Exception ex)
        {
            ...
            return (false, "Identity service unavailable.");
        }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            getRes = await http.SendAsync(getReq, ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            putRes = await http.SendAsync(putReq, ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task<(bool Success, string? ErrorMessage)> TryUpdateUserEmailAsync(
        Guid userId,
        string newEmail,
        CancellationToken ct = default)
    {
        ...
        var userUrl = $"{baseUrl}/admin/realms/{Uri.EscapeDataString(_settings.Realm)}/users/{userId:D}";

        using var getReq = new HttpRequestMessage(HttpMethod.Get, userUrl);
        getReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);

        HttpResponseMessage getRes;
        try { getRes = await http.SendAsync(getReq, ct).ConfigureAwait(false); }
        catch (Exception ex) { _logger.LogError(ex, "Keycloak GET user failed for {UserId}", userId); return (false, ex.Message); }
        ...
        HttpResponseMessage putRes;
        try { putRes = await http.SendAsync(putReq, ct).ConfigureAwait(false); }
        catch (Exception ex) { _logger.LogError(ex, "Keycloak PUT user email failed for {UserId}", userId); return (false, ex.Message); }
        ...
        var body = await putRes.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
        _logger.LogWarning("Keycloak PUT email {Status}: {Body}", (int)putRes.StatusCode, body);
        return (false, $"Keycloak returned {(int)putRes.StatusCode}.");
    }


    private async Task<string?> RequestTokenAsync(HttpClient http, string baseUrl, CancellationToken ct)
    {
        var tokenUrl =
        ...
        try
        {
            res = await http.PostAsync(tokenUrl, content, ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
