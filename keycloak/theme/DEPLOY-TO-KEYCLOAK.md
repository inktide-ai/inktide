...
### Session & token times — **15 minutes** (login + emails)

Chimera email templates say *“This link expires in {0}.”* — **`{0}` is generated by Keycloak** from realm settings, not hardcoded in the theme. To use **15 minutes everywhere** (same idea as **Login action timeout**):

| Where (Realm settings) | Setting | Value |
|------------------------|---------|--------|
| **Sessions** | **Login action timeout** | **15** Minutes |
| **Tokens** | **User-Initiated Action Lifespan** (and any separate **Email verification** / action-token lifespan if shown in your Keycloak version) | **15** Minutes |

Defaults are often **5** minutes for user-initiated email links; raising them to **15** makes the text in password-reset / verify-email messages match a 15-minute **Login action timeout** policy.

## 5.2 Optional: delete account from Chimera API (Keycloak Admin)

The API can remove the Keycloak user after purging Soul data. Configure in `appsettings.json`:
...
"KeycloakAdminSettings": {
  ...
  "Realm": "chimera-app",
  "ClientId": "chimera-api-admin",
  "ClientSecret": "<secret>"
}
```

In Keycloak Admin: create a **confidential** client in the same realm, enable **Client authentication**, enable **Service accounts roles**, then under **Service account roles** assign **realm-management** → `manage-users` (and `view-users` if required). Use that client’s id/secret above. If `Enabled` is `false`, only PostgreSQL data is removed; the user must delete the identity in Keycloak manually.
